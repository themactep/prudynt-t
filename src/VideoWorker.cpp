...
void VideoWorker::run()
{
    ...
    auto flush_mp4_sample = [&](bool recorder_active, bool live_sinks_active) {
        ...
        bool waiting_for_idr = video_state
                                    ? video_state->mp4_waiting_for_idr.load(
        ...
        if (waiting_for_idr)
        {
            ...
            int64_t required_ts = video_state
                                       ? video_state->mp4_required_idr_ts.load(
                                             std::memory_order_relaxed)
                                       : -1;
    ...
    while (global_video[encChn]->running)
    {
        ...
        if (video_state && video_state->mp4_waiting_for_idr.load(std::memory_order_relaxed))
        {
            ...
            uint64_t last_req = video_state->mp4_last_idr_request_ms.load(
                std::memory_order_relaxed);
    ...
    || defined(PLATFORM_T23)
                    ...
                    if (need_samples && payload_len > 0 && !(nal_is_sps || nal_is_pps))
                    {
                        ...
                        bool waiting_for_idr_flag = video_state
                                               ? video_state->mp4_waiting_for_idr.load(
                                                   std::memory_order_relaxed)
                                               : false;
                        if (mp4_sample.empty() && waiting_for_idr_flag && !mp4_inserted_codec_config
                            && video_state)
                        {
