...
std::vector<ConfigItem<const char *>> CFG::getCharItems() {
	return {
		...
		{"stream0.osd.logo_path", stream0.osd.logo_path, "/usr/share/thingino_logo_0.bgra", validateCharNotEmpty},
		...
		{"stream0.profile", stream0.profile, "SMART", [](const char *v) {
		    std::set<std::string> a = {"CBR", "VBR", "SMART", "FIXQB",};
		    return a.count(std::string(v)) == 1;
		}},
...
std::vector<ConfigItem<int>> CFG::getIntItems() {
	return {
		...
		{"stream0.osd.logo_height", stream0.osd.logo_height, 60, [](const int &v) { return v > 0; }},
		...
		{"stream0.osd.logo_width", stream0.osd.logo_width, 224, [](const int &v) { return v > 0; }},
...
bool findSetting(const std::string &path, libconfig::Setting *&foundSetting, const libconfig::Setting *root) {
