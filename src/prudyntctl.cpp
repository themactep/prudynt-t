...
static int cmd_json(int argc, char **argv) {
  ...
  // Important: signal EOF on the write side so server stops reading and
  // processes
  shutdown(fd, SHUT_WR);
...
static int cmd_snapshot(int argc, char **argv) {
  ...
  // Stream exactly len bytes to stdout (server now sends clean JPEG starting
  // at SOI)
  std::vector<char> buf(8192);
  int remaining = len;
  while (remaining > 0) {
