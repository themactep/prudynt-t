...
void BackchannelServerMediaSubsession::getStreamParameters(
  ...
  if (clientRTPPort.num() == 0 && tcpSocketNum < 0) {
    LOG_ERROR("getStreamParameters: Invalid parameters (no client ports or TCP "
              "socket) for session "
              << clientSessionId);
    return;
  }
  ...
  if (!isTCP) { // UDP
    if (clientRTCPPort.num() == 0) {
      // This might be okay if the client doesn't intend to send RTCP Sender
      // Reports
      LOG_WARN("Client requested UDP streaming but provided no RTCP port for "
               "session "
               << static_cast<unsigned>(clientSessionId));
    }
    ...
    if (rtpGroupsock == nullptr || rtpGroupsock->socketNum() < 0) {
      LOG_ERROR("getStreamParameters: Failed to create dummy RTP Groupsock for "
                "TCP session "
                << clientSessionId);
      Medium::close(mediaSink);
    ...
    if (rtcpGroupsock == nullptr || rtcpGroupsock->socketNum() < 0) {
      LOG_ERROR("getStreamParameters: Failed to create dummy RTCP Groupsock "
                "for TCP session "
                << clientSessionId);
      Medium::close(mediaSink);
      delete rtpGroupsock;
