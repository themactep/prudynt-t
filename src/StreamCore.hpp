...
enum class StreamStartPolicy { LiveEdge, LatestSync };

template <typename FrameType> struct StreamCoreTraits {
  static bool is_sync(const FrameType &) {
    return false;
  }
};

template <typename FrameType> class StreamCore {
public:
  struct Cursor {
  ...
  bool waitReadFor(Cursor &cursor, FrameType *out,
...
private:
  ...
  void normalizeCursorLocked(Cursor &cursor) {
    ...
    if (cursor.nextSequence < frontSequence) {
      ...
      if (cursor.startPolicy == StreamStartPolicy::LiveEdge &&
          !cursor.requireSync) {
