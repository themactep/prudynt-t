...
static uint8_t identifySamplingFrequencyIndex(unsigned samplingFrequency) {
  ...
  static constexpr unsigned frequencies[numFrequencies] = {96000, 88200, 64000, 48000, 44100, 32000, 24000,
...
AACSink *AACSink::createNew(UsageEnvironment &env, Groupsock *RTPgs, u_int8_t rtpPayloadFormat,
...
AACSink::AACSink(UsageEnvironment &env, Groupsock *RTPgs, u_int8_t rtpPayloadFormat, u_int32_t rtpTimestampFrequency,
                 unsigned numChannels)
    : MPEG4GenericRTPSink(env, RTPgs, rtpPayloadFormat, rtpTimestampFrequency, "audio", "AAC-hbr",
                          config = strdup(generateConfig(rtpTimestampFrequency, numChannels).c_str()), numChannels) {
}
