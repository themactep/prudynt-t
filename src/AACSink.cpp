...
static uint8_t identifySamplingFrequencyIndex(unsigned samplingFrequency) {
  ...
  static constexpr unsigned frequencies[numFrequencies] = {
...
static std::string generateConfig(unsigned samplingFrequency,
                                  unsigned numChannels) {
  // See RFC 3640
  unsigned samplingFrequencyIndex =
      identifySamplingFrequencyIndex(samplingFrequency);
  constexpr uint8_t objectType = 2; // AAC-LC
  uint8_t channelConfiguration = static_cast<uint8_t>(numChannels);

  uint16_t combined = (objectType << 11) | (samplingFrequencyIndex << 7) |
...
AACSink *AACSink::createNew(UsageEnvironment &env, Groupsock *RTPgs,
                            ...
                            unsigned numChannels) {
  return new AACSink(env, RTPgs, rtpPayloadFormat, rtpTimestampFrequency,
                     numChannels);
}
...
AACSink::AACSink(UsageEnvironment &env, Groupsock *RTPgs,
                 ...
                 unsigned numChannels)
    : MPEG4GenericRTPSink(
          env, RTPgs, rtpPayloadFormat, rtpTimestampFrequency, "audio",
          "AAC-hbr",
          config = strdup(
              generateConfig(rtpTimestampFrequency, numChannels).c_str()),
          numChannels) {
}
