...
int imp_control_get_ev_attributes(char *buffer, int size) {
  if (!buffer || size < (int)sizeof(IMPISPEVAttr))
    return -1;
  IMPISPEVAttr attr{};
...
int imp_control_get_ae_attributes(char *buffer, int size) {
  if (!buffer || size < (int)sizeof(IMPISPAEAttr))
    return -1;
  IMPISPAEAttr attr{};
...
int imp_control_osd_get_region_attr(int handle, char *buffer, int size) {
  if (handle < 0 || !buffer || size < (int)sizeof(IMPOSDRgnAttr))
    return -1;
  IMPOSDRgnAttr attr{};
