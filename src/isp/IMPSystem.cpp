...
void resolve_all_stream_geometry() {
  ...
  // follow that source's resolved size instead of the sensor.
  const _stream &src2 =
      (cfg->stream2.jpeg_channel == 0) ? cfg->stream0 : cfg->stream1;
  const _stream &src3 =
      (cfg->stream3.jpeg_channel == 0) ? cfg->stream0 : cfg->stream1;
