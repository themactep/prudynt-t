...
int TimestampManager::initialize() {
  ...
  LOG_INFO(
      "TimestampManager initialized - using IMP hardware timestamps directly");
  return 0;
}
...
void TimestampManager::getTimestamp(struct timeval *tv) {
  ...
  // TIMESTAMP DEBUG: Log TimestampManager output (improved rate limiting for
  // sync debugging)
  ...
  // Log first 20 calls, then every 1 second, or if there's a significant
  // timestamp jump
  static int64_t last_timestamp = 0;
  int64_t timestamp_diff = std::abs(impTimestamp - last_timestamp);
  bool significant_jump =
      (last_timestamp > 0 && timestamp_diff > 100000); // >100ms jump

  if (log_count < 20 || time_since_last.count() >= 1000 || significant_jump) {
    LOG_DEBUG("TIMESTAMP_MANAGER_SOURCE: impTimestamp="
              << impTimestamp << " tv_sec=" << tv->tv_sec
              << " tv_usec=" << tv->tv_usec
