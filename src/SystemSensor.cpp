...
#else
#define SYSTEM_SENSOR_LOG_DEBUG(msg)                                           \
  std::cout << "DEBUG SystemSensor: " << msg << std::endl
#define SYSTEM_SENSOR_LOG_ERROR(msg)                                           \
  std::cerr << "ERROR SystemSensor: " << msg << std::endl
#define SYSTEM_SENSOR_LOG_INFO(msg)                                            \
  std::cout << "INFO SystemSensor: " << msg << std::endl
#endif
...
SystemSensor::SensorInfo SystemSensor::getSensorInfo() {
  ...
  if (!isAvailable()) {
    throw std::runtime_error(
        "Sensor proc filesystem /proc/jz/sensor/ is not accessible");
  }
  ...
  SYSTEM_SENSOR_LOG_INFO("Successfully retrieved sensor info: "
                         << info.name << " (" << info.width << "x"
                         << info.height << "@" << info.max_fps << "fps)");
...
bool SystemSensor::isAvailable() {
  ...
  return std::filesystem::exists(SENSOR_PROC_DIR) &&
         std::filesystem::is_directory(SENSOR_PROC_DIR);
}
...
int SystemSensor::readProcInt(const std::string &filename, int defaultValue) {
  ...
  if (value.empty()) {
    SYSTEM_SENSOR_LOG_DEBUG("Using default value " << defaultValue << " for "
                                                   << filename);
    return defaultValue;
  }
  ...
  try {
    ...
    SYSTEM_SENSOR_LOG_ERROR("Failed to parse '" << value << "' as int from "
                                                << filename << ": "
                                                << e.what());
    return defaultValue;
  }
...
unsigned int SystemSensor::parseHexString(const std::string &hexStr) {
  ...
  try {
    ...
    SYSTEM_SENSOR_LOG_ERROR("Failed to parse hex string '"
                            << hexStr << "': " << e.what());
    return 0;
  }
}
