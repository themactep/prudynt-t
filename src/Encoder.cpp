...
IMPSensorInfo Encoder::create_sensor_info(const std::string& sensor) {
	IMPSensorInfo out;
		...
		while (cfg->encoder_thread_signal.load() & 2) {
			...
			for (CHN &channel: channels) {
				...
				if (channel.enabled) {
					...
					int64_t nalTsDelta = nalTs - channel.lastNalTs;
					int64_t natOneFrame = (ONESECOND / cfg->stream0.fps * 3 / 2);

					if (nalTsDelta > natOneFrame) {
						// Silence for now until further tests / THINGINO
						// LOG_WARN("The encoder 0 dropped a frame. " << nalTsDelta << ", " << natOneFrame);
					}

					struct timeval encodeTime{};
					encodeTime.tv_sec = nalTs / ONESECOND;
					encodeTime.tv_usec = nalTs % ONESECOND;
					...
					for (unsigned int i = 0; i < stream.packCount; ++i) {
...
#else
						auto *start = (uint8_t *) stream.pack[i].virAddr;
						...
						nalu.duration = 0;
#if defined(PLATFORM_T31)
						if (stream.pack[i].nalType.h264NalType == 1 ||
						...
						for (auto &[sinkId, sink]: Encoder::sinks) {
							if (sink.encChn == channel.encChn) {
								if (!sink.IDR) {
#if defined(PLATFORM_T31)
									if (stream.pack[i].nalType.h264NalType == 5 ||
									    stream.pack[i].nalType.h264NalType == 7 ||
									...
									if (stream.pack[i].dataType.h264Type == 5 ||
									    stream.pack[i].dataType.h264Type == 7 ||
									    stream.pack[i].dataType.h264Type == 8 ||
									    stream.pack[i].dataType.h265Type == 32 ) {
										sink.IDR = true;
									}
#else
									if (stream.pack[i].dataType.h264Type == 5 ||
