			    ...
			    else if (stream0.pack[i].dataType.h265Type == 32)
			    {
							...
							if (sink.IDR) {
								...
								if (sink.data_available_callback(nalu0)) {
									...
									LOG_ERROR("stream 0, eC:" << sink.encChn << ", id:" << sinkId << ", size:" << nalu0.data.size() << ", pC:" << stream0.packCount << ", pS:" << nalu0.data.size() << ", pN:" << i
			    ...
			    else if (stream1.pack[i].dataType.h265Type == 32)
			    {
							...
							if (sink.IDR) {
								if (sink.data_available_callback(nalu1)) {
									...
									LOG_ERROR("stream 0, eC:" << sink.encChn << ", id:" << sinkId << ", size:" << nalu1.data.size() << ", pC:" << stream1.packCount << ", pS:" << nalu1.data.size() << ", pN:" << i
												  << ", sC:" << Encoder::sinks.size() << " clogged!");
								}
							}
						}
