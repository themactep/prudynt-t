...
void write_bits(std::vector<uint8_t> &data, size_t bit_pos, uint32_t value,
                int count) {
...
bool patch_h264_sps_timing(std::vector<uint8_t> &nal, int fps,
  ...
  if (!br.read_bits(static_cast<int>(nal_header_bits), tmp) ||
      !br.read_bits(8, tmp) || !br.read_bits(8, tmp) || !br.read_bits(8, tmp))
    return false;
  if (!br.read_ue(tmp))
    return false;
  ...
  if (profile_idc == 100 || profile_idc == 110 || profile_idc == 122 ||
    ...
    if (chroma_format_idc == 3 && !br.read_bits(1, tmp))
      return false;
    if (!br.read_ue(tmp) || !br.read_ue(tmp) || !br.read_bits(1, tmp) ||
        !br.read_bits(1, tmp))
      return false;
    if (tmp) {
  ...
  if (pic_order_cnt_type == 0) {
    ...
    int32_t stmp = 0;
    if (!br.read_bits(1, tmp) || !br.read_se(stmp) || !br.read_se(stmp) ||
        !br.read_ue(tmp))
      return false;
  ...
  if (!br.read_ue(tmp) || !br.read_bits(1, tmp) || !br.read_ue(tmp) ||
      !br.read_ue(tmp))
    return false;
  ...
  if (!br.read_bits(1, tmp))
    return false;
  if (tmp) {
    if (!br.read_ue(tmp) || !br.read_ue(tmp) || !br.read_ue(tmp) ||
        !br.read_ue(tmp))
      return false;
  }

  uint32_t vui_parameters_present_flag = 0;
  if (!br.read_bits(1, vui_parameters_present_flag) ||
  ...
  if (!br.read_bits(1, timing_info_present_flag) ||
