...
void AudioWorker::process_audio_frame_direct(IMPAudioFrame &frame) {
  ...
  if (frame.timeStamp > 0) {
    ...
    LOG_DEBUG("AUDIO_TIMESTAMP_ACCUMULATED: using frame.timeStamp="
              << frame.timeStamp << " tv_sec=" << encoder_time.tv_sec
              << " tv_usec=" << encoder_time.tv_usec);
  } else {
    // Direct frame - use fresh timestamp
    TimestampManager::getInstance().getTimestamp(&encoder_time);
    LOG_DEBUG("AUDIO_TIMESTAMP_DIRECT: fresh timestamp tv_sec="
              << encoder_time.tv_sec << " tv_usec=" << encoder_time.tv_usec);
  }
  ...
  if (!af.data.empty() && global_audio[encChn]->hasDataCallback &&
    ...
    if (!global_audio[encChn]->msgChannel->write(af)) {
      ...
      LOG_DDEBUG("audio encChn:" << encChn << ", size:" << af.data.size()
                                 << " clogged!");
#else
      LOG_ERROR("audio encChn:" << encChn << ", size:" << af.data.size()
                                << " clogged!");
#endif
    } else {
...
void AudioWorker::process_frame(IMPAudioFrame &frame) {
  ...
  if (global_audio[encChn]->imp_audio->format == IMPAudioFormat::OPUS &&
      targetSamplesPerChannel > 0) {
    int samplesPerChannel = (frame.len / sizeof(int16_t)) /
    ...
    if (frameBuffer.empty()) {
      ...
      // AUDIO SYNC DEBUG: Always log frame accumulation start for sync
      // debugging
      LOG_DEBUG("AUDIO_SYNC_ACCUMULATION_START: "
                << samplesPerChannel
                << " samples per channel, timestamp=" << bufferStartTimestamp);
    }
    ...
    int predictedSamplesPerChannel =
        currentSamplesPerChannel + incomingSamplesPerChannel;

    if (warnBufferSamplesPerChannel > 0 &&
        predictedSamplesPerChannel >= warnBufferSamplesPerChannel) {
      // Rate-limit buffer capacity warnings to avoid log spam
      static auto lastWarnTime = std::chrono::steady_clock::now();
      auto now = std::chrono::steady_clock::now();
      auto timeSinceLastWarn =
          std::chrono::duration_cast<std::chrono::seconds>(now - lastWarnTime);

      if (timeSinceLastWarn.count() >= 5) { // Warn at most every 5 seconds
        LOG_WARN("AudioWorker buffer nearing capacity: "
                 << predictedSamplesPerChannel << "/"
                 << maxBufferSamplesPerChannel << " samples/ch ("
                 << bufferDropCount.load() << " drops so far)");
        lastWarnTime = now;
      }
    }

    // Drop oldest samples to keep within capacity
    while (maxBufferSamplesPerChannel > 0 &&
           predictedSamplesPerChannel > maxBufferSamplesPerChannel &&
           !frameBuffer.empty()) {
      int dropSamplesPerChannel =
          std::max(targetSamplesPerChannel,
                   predictedSamplesPerChannel - maxBufferSamplesPerChannel);
      int dropTotalSamples = dropSamplesPerChannel * outCh;
      dropTotalSamples = std::min(dropTotalSamples, (int)frameBuffer.size());
      frameBuffer.erase(frameBuffer.begin(),
                        frameBuffer.begin() + dropTotalSamples);
      ...
      bufferStartTimestamp +=
          (int64_t)((dropSamplesPerChannel * 1000000LL) /
      ...
      // Expose metrics via RTSPStatus
      {
        ...
        RTSPStatus::writeCustomParameter(
            streamName, "buffer_drop_count",
            std::to_string(bufferDropCount.load()));
        RTSPStatus::writeCustomParameter(
            streamName, "buffer_level_samples_per_channel",
            std::to_string(predictedSamplesPerChannel));
      }
      LOG_WARN("AudioWorker dropped " << dropSamplesPerChannel
                                      << " samples/ch to bound buffer");
    }
    ...
    while (currentSamplesPerChannel >= targetSamplesPerChannel) {
      ...
      int targetTotalSamples =
          targetSamplesPerChannel * global_audio[encChn]->imp_audio->outChnCnt;
      ...
      // Keep original timestamp - the monotonic PTS will be applied in
      // process_audio_frame_direct
      opusFrame.timeStamp = bufferStartTimestamp;

      // AUDIO SYNC DEBUG: Always log frame ready for sync debugging
      LOG_DEBUG("AUDIO_SYNC_FRAME_READY: accumulated "
                << currentSamplesPerChannel << " samples per channel, sending "
      ...
      if (analysis_count < 5) {
        ...
        LOG_DEBUG("Raw PCM analysis "
                  << analysis_count << ": min=" << min_val
      ...
      frameBuffer.erase(frameBuffer.begin(),
                        frameBuffer.begin() + targetTotalSamples);

      // Update timestamp for next frame
      bufferStartTimestamp += (targetSamplesPerChannel * 1000000LL) /
                              global_audio[encChn]->imp_audio->sample_rate;

      // Recalculate remaining samples for next iteration
      currentSamplesPerChannel =
          frameBuffer.size() / global_audio[encChn]->imp_audio->outChnCnt;
    }
...
void AudioWorker::run() {
  ...
  // RFC 7587 COMPLIANCE NOTE: OPUS RTP timestamps MUST always use 48kHz clock
  // rate for signaling purposes, but the actual input sampling rate can be
  // different (8kHz, 16kHz, 24kHz, 48kHz, etc.). The frame accumulator must
  // collect samples based on the ACTUAL input sample rate, not the RTP clock
  // rate. For 20ms frames: required_samples = actual_input_rate * 0.020
  if (global_audio[encChn]->imp_audio->format == IMPAudioFormat::OPUS) {
    // Calculate samples for 20ms at the actual input sample rate
    targetSamplesPerChannel =
        global_audio[encChn]->imp_audio->sample_rate * 0.020;
    frameBuffer.clear();
    ...
    LOG_DEBUG("Opus frame accumulator initialized: target="
              << targetSamplesPerChannel << " samples per channel (20ms at "
              << global_audio[encChn]->imp_audio->sample_rate << "Hz), "
              << "warn@" << warnBufferSamplesPerChannel << ", cap@"
              << maxBufferSamplesPerChannel);
    // Expose initial metrics and thresholds
    {
      std::string streamName = std::string("audio") + std::to_string(encChn);
      RTSPStatus::writeCustomParameter(
          streamName, "buffer_warn_samples_per_channel",
          std::to_string(warnBufferSamplesPerChannel));
      RTSPStatus::writeCustomParameter(
          streamName, "buffer_cap_samples_per_channel",
          std::to_string(maxBufferSamplesPerChannel));
      RTSPStatus::writeCustomParameter(streamName, "buffer_drop_count",
                                       std::to_string(bufferDropCount.load()));
    }
  }
  ...
  while (global_audio[encChn]->running) {
    if (global_audio[encChn]->hasDataCallback && cfg->audio.input_enabled &&
        (global_video[0]->hasDataCallback ||
      ...
      if (IMP_AI_PollingFrame(global_audio[encChn]->devId,
        ...
        // Hardware timestamps are already monotonic (from
        // IMP_System_RebaseTimeStamp) Use them directly - no conversion needed
        // with 64-bit approach DeepSeek's recommendation: use hardware
        // timestamps as-is since they're already monotonic

        if (reframer) {
          // SINGLE SOURCE OF TRUTH: Use TimestampManager timestamp
          uint64_t unified_timestamp =
              TimestampManager::getInstance().getTimestampUs();
...
void *AudioWorker::thread_entry(void *arg) {
  ...
  try {
    global_audio[encChn]->imp_audio = IMPAudio::createNew(
        ...
        global_audio[encChn]->aeChn);
    // Keep global devId in sync with actual initialized device (IMPAudio may
    // adjust devId on some platforms)
    if (global_audio[encChn]->imp_audio &&
        global_audio[encChn]->devId != global_audio[encChn]->imp_audio->devId) {
      LOG_INFO("AudioWorker: remapping devId from "
               << global_audio[encChn]->devId << " to "
               << global_audio[encChn]->imp_audio->devId
               << " based on hardware init");
      global_audio[encChn]->devId = global_audio[encChn]->imp_audio->devId;
    }
