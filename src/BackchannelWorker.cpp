...
BackchannelWorker::resampleLinear(const std::vector<int16_t> &input_pcm,
  ...
  if (input_rate == output_rate) {
    LOG_ERROR("resampleLinear called with equal rates ("
              << input_rate << "), returning input unchanged");
    return input_pcm;
  }
...
bool BackchannelWorker::decodeFrame(const uint8_t *payload, size_t payloadSize,
  ...
  if (ret != 0) {
    ...
    if (global_backchannel && global_backchannel->imp_backchannel) {
      int ensure_ret =
          global_backchannel->imp_backchannel->ensureDecoderChannel(format);
      if (ensure_ret == 0) {
        retried = true;
    ...
    if (ret != 0) {
      LOG_ERROR("IMP_ADEC_SendStream failed for channel "
                << adChn << ": " << ret
...
void BackchannelWorker::signalShutdown() {
  ...
  if (!enqueued) {
    LOG_WARN("Backchannel shutdown sentinel enqueued after dropping oldest "
             "frame (queue was full).");
  }
