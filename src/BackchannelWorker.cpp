...
bool BackchannelWorker::writePcmToPipe(const std::vector<int16_t> &pcmBuffer) {
  ...
  if (bytesWritten == static_cast<ssize_t>(bytesToWrite)) {
    ...
    if (saved_errno == EAGAIN || saved_errno == EWOULDBLOCK) {
      ...
      LOG_ERROR("write() failed: errno=" << saved_errno << ": "
  ...
  while (global_backchannel->running) {
    ...
    if (frame.payload.empty()) {
      ...
      if (frame.clientSessionId == currentSessionId && currentSessionId != 0) {
        LOG_INFO("Current session " << currentSessionId
                                    << " stopped. Closing pipe.");
        closePipe();
        currentSessionId = 0;
    ...
    if (currentSessionId == 0) {
      ...
      if (!processFrame(frame)) {
        ...
        LOG_WARN("processFrame failed for initial frame of session "
                 << currentSessionId << ". Pipe closed.");
        currentSessionId = 0;
      }
      ...
      if (!fPipe) { // Ensure pipe is open (it might have closed unexpectedly)
        LOG_WARN("Pipe was closed unexpectedly for current session "
                 << currentSessionId << ". Reopening.");
        if (!initPipe()) {
