...
#else
#define RTSP_STATUS_LOG_DEBUG(msg)                                             \
  std::cout << "DEBUG RTSPStatus: " << msg << std::endl
#define RTSP_STATUS_LOG_ERROR(msg)                                             \
  std::cerr << "ERROR RTSPStatus: " << msg << std::endl
#define RTSP_STATUS_LOG_INFO(msg)                                              \
  std::cout << "INFO RTSPStatus: " << msg << std::endl
#endif
...
bool RTSPStatus::initialize() {
  ...
  RTSP_STATUS_LOG_DEBUG("Initializing RTSP status interface at "
                        << STATUS_BASE_DIR);

  if (!ensureBaseDirectory()) {
    RTSP_STATUS_LOG_ERROR(
        "Failed to create base directory: " << STATUS_BASE_DIR);
    return false;
  }
...
bool RTSPStatus::updateStreamStatus(const std::string &streamName,
  ...
  if (!createStreamDirectory(streamName)) {
    RTSP_STATUS_LOG_ERROR(
        "Failed to create stream directory for: " << streamName);
    return false;
  }
  ...
  if (success) {
    ...
    RTSP_STATUS_LOG_INFO("Updated status for stream "
                         << streamName << " (" << info.format << " "
                         << info.width << "x" << info.height << "@" << info.fps
                         << "fps)");
  } else {
    RTSP_STATUS_LOG_ERROR(
        "Failed to update some parameters for stream: " << streamName);
  }
...
bool RTSPStatus::createStreamDirectory(const std::string &streamName) {
  ...
  try {
    ...
    RTSP_STATUS_LOG_ERROR("Failed to create stream directory "
                          << streamDir << ": " << e.what());
    return false;
  }
}
...
bool RTSPStatus::writeParameter(const std::string &streamName,
  ...
  try {
    ...
    RTSP_STATUS_LOG_DEBUG("Wrote " << parameter << "=" << value << " to "
                                   << filePath);
    return true;
  } catch (const std::exception &e) {
    RTSP_STATUS_LOG_ERROR("Failed to write parameter " << parameter << " to "
                                                       << filePath << ": "
                                                       << e.what());
    return false;
  }
...
bool RTSPStatus::removeStreamDirectory(const std::string &streamName) {
  ...
  try {
    ...
    RTSP_STATUS_LOG_ERROR("Failed to remove stream directory "
                          << streamDir << ": " << e.what());
    return false;
  }
}
...
bool RTSPStatus::ensureBaseDirectory() {
  try {
    ...
    RTSP_STATUS_LOG_ERROR("Failed to create base directory "
