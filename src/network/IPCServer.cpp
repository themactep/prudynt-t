...
#include "config/JsonAPI.hpp" // JSON processing via jct
...
int IPCServer::handle_client(int fd) {
  ...
  if (starts_with(req, "MJPEG")) {
    ...
    if (w > 0 || h > 0) {
      ...
      // dimension stays closest to the source aspect ratio. Values pass
      // through otherwise: no minimum is imposed, and no w/h at all
      // leaves the encoder at its configured size.
      w = ((w + 8) / 16) * 16;
      if (w < 16)
        w = 16;
      h = ((h + 8) / 16) * 16;
