...
public:
    ...
    // Return the init segment (ftyp+moov or equivalent) to send immediately to clients
    virtual std::vector<uint8_t> getInitSegment() = 0;

    // Feed one video sample (raw NALs for H.264) and return any generated fragment bytes
    // pts is in milliseconds.
    virtual std::vector<uint8_t> muxVideo(const uint8_t* data, size_t size, int64_t pts_ms, bool isKey) = 0;

    // Feed one audio sample (raw AAC frame payload) and return any generated fragment bytes
