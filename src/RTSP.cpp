...
void RTSP::addSubsession(int chnNr, _stream &stream) {
  ...
  auto deviceSource = IMPDeviceSource<H264NALUnit, video_stream>::createNew(
      *env, chnNr, global_video[chnNr], "video/pps/sps/vps");
  H264NALUnit sps;
  H264NALUnit pps;
  ...
  while (!have_pps || !have_sps || (is_h265 && !have_vps)) {
    ...
    if (is_h265) {
      uint8_t nalType =
          (unit.data[0] & 0x7E) >> 1; // H265 NAL unit type extraction
...
#define ADD_BACKCHANNEL_SUBSESSION(EnumName, NameString, PayloadType,          \
                                   Frequency, MimeType)                        \
  {                                                                            \
...
void RTSP::start() {
  ...
  // Generate a single timestamp base that both audio and video RTPSinks will
  // use This eliminates the massive A-V sync differences caused by random
  // timestamp bases
  uint32_t sharedTimestampBase = our_random32();
  char timestampBaseStr[16];
  snprintf(timestampBaseStr, sizeof(timestampBaseStr), "%u",
           sharedTimestampBase);
  setenv("PRUDYNT_SHARED_TIMESTAMP_BASE", timestampBaseStr, 1);

  char logMsg[64];
  snprintf(logMsg, sizeof(logMsg), "Set shared RTP timestamp base: %u (0x%08x)",
           sharedTimestampBase, sharedTimestampBase);
  LOG_INFO(logMsg);
