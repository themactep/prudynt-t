...
int OSD::drawText(uint8_t *image, const char *text, int WIDTH, int HEIGHT,
  ...
  LOG_DEBUG("OSD Text Alpha - Text: " << (int)textColor[3]
                                      << ", Stroke: " << (int)strokeColor[3]);
...
void OSD::set_text(OSDItem *osdItem, IMPOSDRgnAttr *irgnAttr, const char *text,
                   int posX, int posY, int angle, unsigned int font_color,
                   unsigned int font_stroke_color) {
  // size and stroke
...
void OSD::init() {
  ...
  if (osd.time_enabled) {
    ...
    set_text(&osdTime, &osdTime.rgnAttr, osd.time_format, osd.pos_time_x,
             osd.pos_time_y, osd.time_rotation, osd.time_font_color,
  ...
  if (osd.uptime_enabled) {
    ...
    set_text(&osdUptm, &osdUptm.rgnAttr, osd.uptime_format, osd.pos_uptime_x,
             osd.pos_uptime_y, osd.uptime_rotation, osd.uptime_font_color,
  ...
  if (osd.logo_enabled) {
    ...
    if ((osd.logo_width * osd.logo_height * 4) == (int)imageSize) {
      ...
      set_pos(&osdLogo.rgnAttr, osd.pos_logo_x, osd.pos_logo_y, logo_width,
              logo_height, stream_width, stream_height);
    } else {
  ...
  if (ltime->tm_sec != last_updated_second) {
    ...
    if (flag != 0) {
      ...
      if ((flag & 1) && osd.time_enabled) {
        ...
        set_text(&osdTime, nullptr, timeFormatted, osd.pos_time_x,
                 osd.pos_time_y, osd.time_rotation, osd.time_font_color,
                 osd.time_font_stroke_color);
      ...
      if ((flag & 2) && osd.user_text_enabled) {
        ...
        set_text(&osdUser, nullptr, user_text.c_str(), osd.pos_user_text_x,
                 osd.pos_user_text_y, osd.user_text_rotation,
                 osd.user_text_font_color, osd.user_text_font_stroke_color);
      ...
      if ((flag & 4) && osd.uptime_enabled) {
        ...
        set_text(&osdUptm, nullptr, uptimeFormatted, osd.pos_uptime_x,
                 osd.pos_uptime_y, osd.uptime_rotation, osd.uptime_font_color,
                 osd.uptime_font_stroke_color);
