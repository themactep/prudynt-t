...
namespace {
std::atomic<uint32_t> g_opusMismatches{0};
}
...
int Opus::open() {
  ...
  encoder = opus_encoder_create(sampleRate, numChn, OPUS_APPLICATION_AUDIO,
                                &opusError);
  ...
  // Highest complexity for quality (CPU usage is negligible at 1ch/48k on this
  // SoC)
  ...
  // Content is typically music/ambience on cams; this helps tuning
  // psychoacoustics
...
int Opus::encode(IMPAudioFrame *data, unsigned char *outbuf, int *outLen) {
  ...
  if (frame_count < 10) {
    LOG_DEBUG("Opus encode frame "
              << frame_count << ": len=" << data->len
  ...
  if (samples_per_channel != expected_samples) {
    ...
    if (samples_per_channel < expected_samples) {
      if (cnt <= 10 || (cnt % 100) == 0) {
        LOG_WARN("Opus underfilled frame: got "
                 << samples_per_channel << ", expected " << expected_samples
                 << " (20ms@" << sampleRate
                 << "Hz) — dropping to preserve framing");
      }
      return -1; // let upstream accumulate more
    } else {
      if (cnt <= 10 || (cnt % 100) == 0) {
        LOG_WARN("Opus oversized frame: got "
                 << samples_per_channel << ", expected " << expected_samples
                 << " (20ms@" << sampleRate
                 << "Hz) — dropping unexpected size");
      }
