...
IMPAudioServerMediaSubsession::createNewStreamSource(unsigned clientSessionId,
  ...
  FramedSource *audioSourceReplica =
      global_audio[audioChn]->streamReplicator->createStreamReplica();
...
RTPSink *IMPAudioServerMediaSubsession::createNewRTPSink(
  ...
  case IMPAudioFormat::OPUS:
    // RFC 7587: Opus over RTP MUST use a 48 kHz RTP timestamp clock, regardless
    // of the encoder's input sample rate. With unified 20 ms packetization,
    // each RTP timestamp MUST advance by 960 ticks (0.020 * 48000) per packet.
    // Application-level frames (IMPAudio/AudioWorker) use wall-clock PTS based
    // on the actual input sample rate for accumulation and pacing; live555 maps
    // these to RTP timestamps using the frequency specified below (48000).
  ...
  // Always advertise 2 channels in SDP for Opus, regardless of actual encoded
  // channels.
  ...
  return SimpleRTPSink::createNew(
      ...
      /* numChannels */ sdpChannels, allowMultipleFramesPerPacket);
}

char const *
IMPAudioServerMediaSubsession::getAuxSDPLine(RTPSink *rtpSink,
                                             FramedSource *inputSource) {
  ...
  if (global_audio[audioChn]->imp_audio->format == IMPAudioFormat::OPUS &&
    ...
    int n = snprintf(buf, sizeof(buf),
                     "a=fmtp:%u stereo=0; sprop-stereo=0; "
                     "maxplaybackrate=48000; maxaveragebitrate=%u\r\n",
                     pt, maxavg);
    if (n > 0) {
      if (fAuxSDPLine) {
        delete[] fAuxSDPLine;
