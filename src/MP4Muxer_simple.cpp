...
public:
    ...
    bool init(const InitParams& params) override {
        ...
        if (!params.aacConfig.empty()) {
            ...
            LOG_INFO("SimpleMP4Muxer: audio enabled sample_rate=" << audio_.timescale
                                                                    << "Hz channels="
                                                                    << audio_.channels
                                                                    << " config_bytes="
                                                                    << audio_.codec_config.size());
        } else {
    ...
    std::vector<uint8_t> muxAudio(const uint8_t* data, size_t size, int64_t pts_ms) override {
        if (!audio_.enabled) {
            if (!audio_warn_no_track_reported_) {
                LOG_WARN("SimpleMP4Muxer: muxAudio called but audio track is not initialized");
        ...
        if (audio_sample_count_ == 1) {
            LOG_INFO("SimpleMP4Muxer: first audio sample accepted seq=" << seq
                                                                         << " pts_ms=" << pts_ms
                                                                         << " bytes=" << size);
        } else if ((audio_sample_count_ % 50) == 0) {
            LOG_DDEBUG("SimpleMP4Muxer(audio): sample #" << audio_sample_count_
                                                          << " seq=" << seq
                                                          << " pts_ms=" << pts_ms
                                                          << " bytes=" << size);
        }
...
private:
    ...
    void write_moov(std::vector<uint8_t>& out) {
        ...
        // mvhd
        {
            ...
            write_u32(mvhd, 0);           // reserved
            // matrix
            write_u32(mvhd, 0x00010000u); write_u32(mvhd, 0);           write_u32(mvhd, 0);
            write_u32(mvhd, 0);           write_u32(mvhd, 0x00010000u); write_u32(mvhd, 0);
            write_u32(mvhd, 0);           write_u32(mvhd, 0);           write_u32(mvhd, 0x40000000u);
            // pre_defined
            for (int i = 0; i < 6; ++i) write_u32(mvhd, 0);
            // next_track_ID
        ...
        // mvex/trex for each track (default fragment values)
        {
            ...
            if (video_.enabled) write_trex(mvex, video_.track_id);
            if (audio_.enabled) write_trex(mvex, audio_.track_id);
    ...
    void write_trak(std::vector<uint8_t>& out, const TrackState& t, bool is_video) {
        ...
        // tkhd
        {
            ...
            // matrix
            write_u32(tkhd, 0x00010000u); write_u32(tkhd, 0);           write_u32(tkhd, 0);
            write_u32(tkhd, 0);           write_u32(tkhd, 0x00010000u); write_u32(tkhd, 0);
            write_u32(tkhd, 0);           write_u32(tkhd, 0);           write_u32(tkhd, 0x40000000u);
            // width/height (16.16)
        ...
        std::vector<uint8_t> stbl;
        {
            ...
            if (is_video) {
                ...
                for (int i = 0; i < 3; ++i) write_u32(avc1, 0);
                ...
                write_u8(avc1, 0);
                for (int i = 0; i < 31; ++i) write_u8(avc1, 0);
    ...
    std::vector<uint8_t> write_fragment(TrackState& t,
                                        uint32_t seq,
                                        const uint8_t* data,
                                        size_t size,
                                        int64_t pts_ms,
        ...
        if (!isVideo)
        {
            ...
            {
                LOG_WARN("SimpleMP4Muxer(audio): non-monotonic PTS (prev=" << prev_pts
                                                                             << " new=" << pts
                                                                             << ")");
            ...
            {
                LOG_WARN("SimpleMP4Muxer(audio): synthesized duration due to missing timing -> "
                         << sample_duration);
        ...
        size_t data_offset_patch_pos = 0;
        {
            ...
            // data_offset bytes live inside the trun payload (after version/flags + sample_count)
            data_offset_patch_pos = traf_box_offset + 8 + trun_box_offset + 16;
        }

        // Now we know moof size; data_offset in trun = moof_size + 8 (size+type of mdat)
        ...
        // For video, assume NAL units are already length-prefixed or Annex-B; we just dump as-is.
        // For real-world use, you may want to convert Annex-B to length-prefixed here.
