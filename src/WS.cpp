...
signed char WS::osd_callback(struct lejp_ctx *ctx, char reason) {
  ...
  if (reason & LEJP_FLAG_CB_IS_VALUE && ctx->path_match) {
    ...
    else if (ctx->path_match >= PNT_OSD_TIME_FONT_COLOR &&
             ctx->path_match <= PNT_OSD_USERTEXT_FONT_STROKE_COLOR) {
      if (reason == LEJPCB_VAL_STR_END) {
...
int WS::ws_callback(struct lws *wsi, enum lws_callback_reasons reason,
  ...
  case LWS_CALLBACK_ESTABLISHED:
    ...
    LOG_DDEBUGWS("LWS_CALLBACK_ESTABLISHED id:" << u_ctx->id
                                                << ", ip:" << client_ip);
  ...
  case LWS_CALLBACK_CLOSED:
    ...
    LOG_DDEBUGWS("LWS_CALLBACK_CLOSED id:" << u_ctx->id << ", ip:" << client_ip
                                           << ", flag:" << u_ctx->flag);
