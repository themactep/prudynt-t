...
static const char *const rtsp_keys[] = {
    "port",          "est_bitrate", "out_buffer_size", "send_buffer_size",
...
static const char *const stream_keys[] = {
    "enabled",      "audio_enabled", "scale_enabled", "rtsp_endpoint",
    "rtsp_info",    "format",        "mode",          "gop",
    "max_gop",      "fps",           "buffers",       "width",
    "height",       "bitrate",       "rotation",      "scale_width",
    "scale_height", "profile",       "stats",         "osd"};
...
static const char *const motion_keys[] = {
    "debounce_time", "post_time",    "cooldown_time", "init_time",
    "min_time",      "thread_wait",  "sensitivity",   "skip_frame_count",
    "frame_width",   "frame_height", "roi_0_x",       "roi_0_x",
    "roi_1_x",       "roi_1_y",      "roi_count",     "enabled",
    "script_path",   "rois"};
...
struct user_ctx {
  ...
  user_ctx(const char *session_id, lws *wsi_handle)
      : wsi(wsi_handle), value(0), flag(0), region(), midx(0), vidx(0),
        post_data_size(0), rx_message(), tx_message(), message(), sul(),
        snapshot() {
        ...
        if (reason == LEJPCB_VAL_NUM_INT) {
          if (cfg->set<int>(u_ctx->path, atoi(ctx->buf))) {
            IMP_ISP_Tuning_SetAntiFlickerAttr(
                (IMPISPAntiflickerAttr)cfg->get<int>(u_ctx->path));
          }
        }
        add_json_num(u_ctx->message, cfg->get<int>(u_ctx->path));
        break;
        ...
        if (reason == LEJPCB_VAL_NUM_INT) {
          if (cfg->set<int>(u_ctx->path, atoi(ctx->buf))) {
            IMP_ISP_Tuning_SetISPRunningMode(
                (IMPISPRunningMode)cfg->get<int>(u_ctx->path));
          }
        }
...
signed char WS::osd_callback(struct lejp_ctx *ctx, char reason) {
  ...
  if (reason & LEJP_FLAG_CB_IS_VALUE && ctx->path_match) {
    ...
    // unsigned int
    else if (ctx->path_match >= PNT_OSD_FONT_COLOR &&
             ctx->path_match <= PNT_OSD_FONT_STROKE_COLOR) {
      if (reason == LEJPCB_VAL_STR_END) {
        if (cfg->set<unsigned int>(u_ctx->path,
          ...
          if (IMP_OSD_GetRgnAttr(3, &rgnAttr) == 0) {
            if (u_ctx->value == 0) {
              OSD::set_pos(&rgnAttr, cfg->stream0.osd.pos_logo_x,
                           cfg->stream0.osd.pos_logo_y, 0, 0,
                           cfg->stream0.width, cfg->stream0.height);
            } else if (u_ctx->value == 1) {
              OSD::set_pos(&rgnAttr, cfg->stream1.osd.pos_logo_x,
                           cfg->stream1.osd.pos_logo_y, 0, 0,
                           cfg->stream1.width, cfg->stream1.height);
            }
...
int WS::ws_callback(struct lws *wsi, enum lws_callback_reasons reason,
  ...
  case LWS_CALLBACK_ESTABLISHED:
    ...
    // Immediately send a small server ack so clients can verify the socket is
    // usable and tools like wscat show incoming data right after connect.
  ...
  case LWS_CALLBACK_RECEIVE:
    ...
    LOG_DDEBUGWS("u_ctx->rx_message: id:" << u_ctx->id
                                          << ", rx:" << u_ctx->rx_message);
    ...
    if (u_ctx->flag & PNT_FLAG_WS_REQUEST_PREVIEW) {
      ...
      if (dur > 1000) {
        ...
        LOG_DDEBUGWS("RPS: " << u_ctx->snapshot.rps << " "
                             << u_ctx->snapshot.throttle << " " << dur);
      }

      int delay = (LWS_USEC_PER_SEC / (global_jpeg[0]->stream->stats.fps +
                                       u_ctx->snapshot.throttle)) +
                  first_request_delay;
      LOG_DDEBUGWS("shedule preview image. id:" << u_ctx->id
                                                << " delay:" << delay);
  ...
  case LWS_CALLBACK_SERVER_WRITEABLE:
    LOG_DDEBUGWS("LWS_CALLBACK_SERVER_WRITEABLE id:" << u_ctx->id
                                                     << ", ip:" << client_ip);
  ...
  case LWS_CALLBACK_HTTP: {
    ...
    if (lws_hdr_copy(wsi, upgrade_hdr, sizeof(upgrade_hdr), WSI_TOKEN_UPGRADE) >
        0) {
      LOG_DEBUG("HTTP request contains Upgrade:'"
                << upgrade_hdr << "' — deferring to WS handshake");
  ...
  case LWS_CALLBACK_HTTP_BODY_COMPLETION: // LWS_CALLBACK_HTTP_BODY:
    LOG_DDEBUGWS("LWS_CALLBACK_HTTP_BODY ip:" << client_ip << ", data:"
                                              << u_ctx->rx_message);
  ...
  case LWS_CALLBACK_HTTP_WRITEABLE:
    LOG_DDEBUGWS("LWS_CALLBACK_HTTP_WRITEABLE ip:" << client_ip << " "
                                                   << (int)u_ctx->flag);
  ...
  case LWS_CALLBACK_HTTP_DROP_PROTOCOL:
    LOG_DDEBUGWS("LWS_CALLBACK_HTTP_DROP_PROTOCOL ip:" << client_ip
                                                       << ", id:" << u_ctx->id);
    u_ctx->~user_ctx();
    break;
...
void WS::start() {
  ...
  if (test_fd >= 0) {
    ...
    if (bind(test_fd, (sockaddr *)&sin, sizeof(sin)) < 0) {
      LOG_ERROR("Port " << cfg->websocket.port << " appears busy: "
  ...
  // Put our websocket protocol first so clients without Sec-WebSocket-Protocol
  // still bind to it
