 ...
 *
 * Provides access to sensor information directly via /proc/jz/sensor/
 * filesystem. This ensures dynamic sensor detection without requiring static
 * JSON configuration files, providing a single source of truth for sensor data
 * directly from the sensor driver.
 *
 * Behavior:
 * - Reads sensor parameters from /proc/jz/sensor/ files
 * - Uses default values for missing individual files
 * - Throws runtime_error if /proc/jz/sensor/ directory is not accessible
 */
class SystemSensor {
public:
  struct SensorInfo {
    ...
    SensorInfo()
        : width(1920), height(1080), min_fps(5), max_fps(30), i2c_bus(0),
          boot(0), mclk(1), video_interface(0), reset_gpio(-1),
