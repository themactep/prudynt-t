...
IMPServerMediaSubsession::IMPServerMediaSubsession(
  ...
  LOG_DEBUG("IMPServerMediaSubsession ctor: ch="
            << encChn << " sps.size=" << this->sps.data.size()
...
IMPServerMediaSubsession::~IMPServerMediaSubsession() {
  LOG_DEBUG("IMPServerMediaSubsession dtor: ch="
            << encChn << " vps_addr=" << reinterpret_cast<uintptr_t>(vps)
...
IMPServerMediaSubsession::createNewStreamSource(unsigned clientSessionId,
                                                unsigned &estBitrate) {
  LOG_DEBUG("Create Stream Source. encChn=" << encChn << " clientSessionId="
                                            << clientSessionId);
  ...
  if (!imp) {
    LOG_ERROR(
        "createNewStreamSource: IMPDeviceSource::createNew returned null for ch"
        << encChn);
    return nullptr;
  }
  ...
  if (vps) {
    framer =
        H265VideoStreamDiscreteFramer::createNew(envir(), imp, false, false);
  } else { // Let's assume the default is H264 if not H265
    framer =
        H264VideoStreamDiscreteFramer::createNew(envir(), imp, false, false);
  }
  LOG_DEBUG("createNewStreamSource: returning framer_addr="
            << reinterpret_cast<uintptr_t>(framer) << " for ch" << encChn);
  return framer;
}
...
RTPSink *IMPServerMediaSubsession::createNewRTPSink(
  ...
  if (sps.data.empty()) {
    LOG_ERROR("createNewRTPSink: SPS data is empty for ch"
              << encChn << " — this will crash!");
  }
  if (pps.data.empty()) {
    LOG_ERROR("createNewRTPSink: PPS data is empty for ch"
              << encChn << " — this will crash!");
  }
  ...
  if (vps) {
    RTPSink *sink = H265VideoRTPSink::createNew(
    ...
    LOG_DEBUG("createNewRTPSink: created H265 sink_addr="
              << reinterpret_cast<uintptr_t>(sink) << " for ch=" << encChn);
    return sink;
  } else {
    // For H264 or other formats, VPS is not used
    RTPSink *sink = H264VideoRTPSink::createNew(
        envir(), rtpGroupsock, rtpPayloadTypeIfDynamic, &sps.data[0],
        sps.data.size(), &pps.data[0], pps.data.size());
    if (sink != nullptr) {
      sink->enableRTCPReports() = False;
    }
    LOG_DEBUG("createNewRTPSink: created H264 sink_addr="
              << reinterpret_cast<uintptr_t>(sink) << " for ch=" << encChn);
    return sink;
  }
...
char const *IMPServerMediaSubsession::sdpLines(int addressFamily) {
  ...
  // restart.
  LOG_DEBUG("sdpLines: ch=" << encChn << " sps.size=" << sps.data.size()
                            << " pps.size=" << pps.data.size());
  if (encChn >= 0 && encChn < NUM_VIDEO_CHANNELS && global_video[encChn]) {
    ...
    if (changed) {
      LOG_DEBUG("sdpLines: codec config changed for ch="
                << encChn << " new sps.size=" << sps.data.size()
                << " pps.size=" << pps.data.size()
  ...
  LOG_DEBUG("sdpLines: base returned addr="
            << reinterpret_cast<uintptr_t>(lines) << " for ch=" << encChn);
  return lines;
}
