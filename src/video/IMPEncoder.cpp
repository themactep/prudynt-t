...
int IMPEncoder::init() {
  ...
  //
  // DISABLED on T31: calling these on any encoder channel corrupts
  // shared hardware state and breaks the sub-stream encoder (ch1).
  // Re-enable once a per-platform safe calling sequence is found.
#if 0
  if (!is_jpeg) {
