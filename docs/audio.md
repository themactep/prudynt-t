...
Prudynt exposes a text-based FIFO at `/run/prudynt/audio_out` whenever `audio.spk_enabled` is true.
Each line written into the FIFO is treated as a standalone command:

- `PLAY url=<url> [format=pcm|wav|aac|opus|mp3|flac] [rate=16000] [vol=NN] [gain=NN] [append=0|1] [loop=1] [delay=0]` – Plays a 16-bit mono PCM file. `path=` is accepted as a backwards-compatible alias for file-based assets, but `url=` better reflects that inputs may live on local storage or remote transports handled by prudynt.
  - WAV files are auto-detected (or `format=wav`) and their embedded sample-rate is honored; raw PCM defaults to the active playback rate (16 kHz or 48 kHz, following `audio.mic_hq`) unless `rate=` is provided;
  - AAC/ADTS files can be auto-detected by their `.aac`/`.adts` suffix or forced with `format=aac`;
  - Opus-in-Ogg assets (`.opus`/`.oga`/`.ogg`) can likewise be auto-detected or explicitly selected with `format=opus`;
  - MP3 assets (`.mp3`, `.mp2`, `.mpeg`) are auto-recognized through their extensions or by scanning for ID3/sync words;
  - FLAC files (`.flac`) are detected by the `fLaC` marker;
  Either format can also be forced via `format=mp3`/`format=flac`.
  If no helpful extension is present the FIFO inspects file contents, looking for RIFF/WAVE, ADTS sync words, Ogg BOS pages with `OpusHead`, FLAC magic, or MP3 sync words to pick the right decoder.
  Non-mono inputs are averaged to mono and resampled to the active playback rate, and codec-specific metadata (Opus pre-skip, FLAC sample-rate, MP3 channel count) is honored before enqueueing.
  When `append=0` (default) the pending queue is cleared before playback.
  `loop=N` re-queues the same asset up to 32 times (default 1) before moving on,
  `delay=MS` inserts up to 5000 ms of silence between loops, and volume/gain options apply before the first chunk.
- `STOP` – Flushes the playback queue and calls `IMP_AO_FlushChnBuf`, effectively interrupting whatever is queued.
- `MUTE <0|1>` – Toggles the hardware mute flag via `IMP_AO_SetVolMute`. Accepts the same forms as other commands (e.g. `MUTE value=on`, `MUTE 0`). When muted, queued samples remain buffered so `MUTE 0` resumes audio from wherever playback currently is.
- `VOLUME <value>` / `GAIN <value>` – Immediate adjustments without requiring audio data; values are clamped to the same ranges accepted in `thingino.json` (volume -30..120, gain 0..31).
- `SET volume=<value> gain=<value>` – Convenience variant that can set one or both parameters on a single line.

All arguments must use `key=value` tokens separated by whitespace (e.g. `PLAY url=/tmp/tada.wav volume=70 loop=2`), mirroring the MP4 control FIFO style. Responses are logged via `AudioOutputControl`, so invoking `tail -f /var/log/prudynt.log` helps diagnose parse failures.
